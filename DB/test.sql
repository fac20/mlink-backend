...
INSERT INTO medical_schools_list(medical_school) VALUES 
...
INSERT INTO postgrad_exam_institutions_list (postgrad_exam_institution) VALUES 
...
CREATE TABLE users (
    ...
    phone_number int,
    current_job_title int REFERENCES job_titles(ID),
    current_job_workplace int REFERENCES workplace_list(ID),
    current_job_start DATE,
    current_job_speciality int REFERENCES specialities(ID),
    previous_job_titles VARCHAR(255) ARRAY,
    previous_job_start DATE ARRAY,
    previous_job_end DATE ARRAY,
    previous_job_speciality int REFERENCES specialities(ID),
    degree_title int REFERENCES degree_titles_list(ID),
    medical_school int REFERENCES medical_schools_list(ID),
    degree_start DATE,
    degree_end DATE,
...
-- we have a table for users postgrad exam
-- now we need to think of how we will be inserting data into it
-- or retreiving data from it when we want to display it 
CREATE TABLE user_postgrad_exams (
    ID int NOT NULL PRIMARY KEY,
    user_id int NOT NULL REFERENCES users(ID),
    exam_id int NOT NULL REFERENCES postgrad_exams_list(ID),
    Institution_id int NOT NULL REFERENCES postgrad_exam_institutions
);

INSERT INTO user_postgrad_exams (user_id, exam_id) VALUES 
    (1, 1),
    (3, 5),
    (7, 1),
    (1, 5),
